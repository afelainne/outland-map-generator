...
export function generateContourLines(
  ...
  for (let y = 0; y < rows; y++) {
    ...
    for (let x = 0; x < cols; x++) {
      ...
      field[y][x] =
        noise2D(nx, ny) * 0.5 +
        noise2D(nx * 2.1, ny * 2.1) * 0.3 +
        noise2D(nx * 4.3, ny * 4.3) * 0.2;
    }
  }
    ...
    for (let i = 0; i < segments.length; i++) {
      ...
      while (changed) {
        ...
        for (let j = 0; j < segments.length; j++) {
          ...
          if (dist(last, seg[0]) < resolution * 1.5) {
            chain.push(seg[1]);
            used.add(j);
            changed = true;
          } else if (dist(last, seg[1]) < resolution * 1.5) {
            chain.push(seg[0]);
            used.add(j);
            changed = true;
          } else if (dist(first, seg[1]) < resolution * 1.5) {
            chain.unshift(seg[0]);
            used.add(j);
            changed = true;
          } else if (dist(first, seg[0]) < resolution * 1.5) {
            chain.unshift(seg[1]);
            used.add(j);
            changed = true;
          }
      ...
      if (chain.length > 3) {
        const simplified = simplify(chain, 3);
